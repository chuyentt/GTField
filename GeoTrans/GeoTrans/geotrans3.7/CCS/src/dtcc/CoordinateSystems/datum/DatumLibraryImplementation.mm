 ...
 */
  ...
  /*  Open the File READONLY, or Return Error Condition:                    */
    ...
    // Hacking Objective-C++
    NSString *dataBundle = [[NSBundle mainBundle] pathForResource:@"geotrans_data" ofType:@"bundle"];
    if(!dataBundle) {
        NSLog(@"Could not find data");
        return;
    }
    
    const char* datumFile7 = [[dataBundle stringByAppendingPathComponent:@"7_param.dat"]
                              fileSystemRepresentation];
    const char* datumFile3 = [[dataBundle stringByAppendingPathComponent:@"3_param.dat"]
                              fileSystemRepresentation];
    //

  if (( fp_7param = fopen( datumFile7, "r" ) ) == NULL)
  {
    char message[256] = "";
  ...
  index ++;
    ...
    if (( fp_3param = fopen( datumFile3, "r" ) ) == NULL)
    {
       char message[256] = "";
       strcpy( message, ErrorMessages::datumFileOpenError );
 ...
 */
  ...
  strcat( FileName, "3_param.dat" );
    ...
    //NSString *datumPath7 = [dataBundle stringByAppendingPathComponent:@"7_param.dat"];
    NSString *datumPath3 = [dataBundle stringByAppendingPathComponent:@"3_param.dat"];
    const char* datumFile3 = [datumPath3 fileSystemRepresentation];
    //

    
  if ((fp_3param = fopen(datumFile3, "w")) == NULL)
  { /* fatal error */
    char message[256] = "";
    if (NULL == PathName)
    {
 ...
 */
  ...
  strcat( FileName, "7_param.dat" );
    ...
    NSString *datumPath7 = [dataBundle stringByAppendingPathComponent:@"7_param.dat"];
    const char* datumFile7 = [datumPath7 fileSystemRepresentation];
    //NSString *datumPath3 = [dataBundle stringByAppendingPathComponent:@"3_param.dat"];
    //const char* datumFile3 = [datumPath3 fileSystemRepresentation];
    //

  if ((fp_7param = fopen(datumFile7, "w")) == NULL)
  { /* fatal error */
    char message[256] = "";
    if (NULL == PathName)
    {
  ...
  /* write file */
  long index = MAX_WGS;
  long endIndex = datum7ParamCount + MAX_WGS;
