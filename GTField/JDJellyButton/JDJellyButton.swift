...
class JDJellyButton
{
    ...
    init(rootView: UIView) {
        ...
        Container = JelllyContainer(frame: frame)
    }
    ...
    func attachtoView(rootView:UIView, mainbutton image:UIImage)
    {
        RootView = rootView
        let frame = rootView.frame
        let MainButtonFrame:CGRect = CGRect(x: (frame.width - buttonWidth)/2, y: frame.height - frame.height / 5, width: buttonWidth, height: buttonHeight)
    ...
    func attachtoView(rootView: UIView, mainbutton: UIImage, frame: CGRect)
    {
        RootView = rootView
        let MainButtonFrame:CGRect = frame
        MainButton = JDJellyMainButton(frame: MainButtonFrame, img: mainbutton, Parent: Container)
        MainButton.rootView = rootView
        MainButton.delegate = self
        Container.addSubview(MainButton)
        rootView.addSubview(Container)
    ...
    func detach() {
        RootView = nil
        MainButton.rootView = nil
        MainButton.delegate = nil
