...
open class AEXMLElement {
    ...
    */
    ...
    // MARK: - XML Write Bổ sung
    
    /**
     Inserts child XML element to `self`.
     
     - parameter child: Child XML element to insert.
     
     - parameter at: insert at index
     
     - returns: Child XML element with `self` as `parent`.
     */
    @discardableResult open func insertChild(_ child: AEXMLElement, at: Int) -> AEXMLElement {
        child.parent = self
